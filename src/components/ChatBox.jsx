...
const ChatBox = ({ chatId }) => {
  ...
  useEffect(() => {
    const markMessagesAsRead = () => {
      ...
      if (unreadMessages.length > 0) {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            unreadMessages.includes(m._id) ? { ...m, read: true } : m
          )
        );
      }
    };
    ...
    const observer = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting) markMessagesAsRead();
    });
  ...
  useEffect(() => {
    socket.on("messagesRead", ({ userId: readerId, messageIds }) => {
      setMessages((prev) =>
        prev.map((m) =>
  ...
  const sendMessage = async (e, retryId = null) => {
    ...
    const msgContent = retryId
      ...
      : trimmed;
    const tempMsg = { _id: tempId, content: msgContent, sender: { _id: userId, username: "You" }, chat: chatId, status: "sending", read: false };
